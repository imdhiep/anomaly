...
def extract_patches_with_coordinates(
    ...
    """
    # Handle both 4D and 5D tensors by squeezing batch dimension if present
    if volume.dim() == 5:
        volume = volume.squeeze(0)  # Remove batch dimension
    elif volume.dim() != 4:
        raise ValueError(f"Expected 4D tensor (C, D, H, W) or 5D tensor (B, C, D, H, W), got {volume.dim()}D")
    ...
    if D < patch_d or H < patch_h or W < patch_w:
        print(f"Warning: Volume ({D}, {H}, {W}) is smaller than patch size ({patch_d}, {patch_h}, {patch_w})")
        # Return a single patch using the entire volume, padded if necessary
        if D < patch_d:
            padding_d = patch_d - D
            volume = F.pad(volume, (0, 0, 0, 0, 0, padding_d))
            D = patch_d
        if H < patch_h:
            padding_h = patch_h - H
            volume = F.pad(volume, (0, 0, 0, padding_h, 0, 0))
            H = patch_h
        if W < patch_w:
            padding_w = patch_w - W
            volume = F.pad(volume, (0, padding_w, 0, 0, 0, 0))
            W = patch_w
    ...
    if len(patches) == 0:
        raise ValueError(f"No patches could be extracted from volume shape {volume.shape} with patch size {patch_size}")
