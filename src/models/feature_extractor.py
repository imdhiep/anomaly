...
class ResNet3D(nn.Module):
    ...
    def __init__(self, block, num_blocks: List[int], in_channels: int = 1, 
        ...
        # Use MaxPool3d with stride=(2,2,1) to avoid pooling in depth dimension
...
class Simple3DCNN(nn.Module):
    ...
    def __init__(self, in_channels: int = 1, feature_dims: List[int] = [32, 64, 128, 256]):
        ...
        for i, dim in enumerate(feature_dims):
            # Use 2D pooling for spatial dimensions and avoid pooling in depth if depth is small
            conv_block = nn.Sequential(
                ...
                nn.BatchNorm3d(dim),
                nn.ReLU(inplace=True),
                # Use MaxPool3d with kernel_size=(2,2,1) to avoid pooling in depth dimension
